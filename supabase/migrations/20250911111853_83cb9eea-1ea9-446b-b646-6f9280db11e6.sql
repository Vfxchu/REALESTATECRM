...
-- Enable RLS on storage.objects if not already enabled (handled by Supabase automatically, commenting out to avoid permission error)
-- ALTER TABLE storage.objects ENABLE ROW LEVEL SECURITY;

-- Property images bucket policies (private)
DROP POLICY IF EXISTS "Agents can view property images for their properties" ON storage.objects;
...
DROP POLICY IF EXISTS "Agents can upload property images for their properties" ON storage.objects;
...
DROP POLICY IF EXISTS "Agents can update property images for their properties" ON storage.objects;
...
DROP POLICY IF EXISTS "Agents can delete property images for their properties" ON storage.objects;
